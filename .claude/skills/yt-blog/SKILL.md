...
본문 구조는 템플릿을 채우는 게 아니라 **영상의 실제 전개를 따른다**. 자막을 읽고 유형을 판단한 뒤 그에 맞는 흐름으로:

| 영상 유형 | 본문 흐름 예시 |
| --- | --- |
| 문제→해결형 팁 | `## 문제 — ...` → `## 해결 — ...` → (부가 팁 섹션) |
| 개념 설명형 | 개념 정의 → 동작 원리 → 예시 |
| 비교/리뷰형 | 선택지 소개 → 판단 기준 → 결론 |
| 튜토리얼형 | 단계별 진행 (1단계, 2단계, ...) |

조건부 섹션 — **기술 패턴을 다루는 영상일 때만** "정리" 앞에 추가:
