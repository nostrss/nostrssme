...
오늘은 모나드라는 개념을 알아보면서 그 내용을 작성했다.

내용을 정리하면서 읽어보니, 강사가 지금까지 강의한 내용의 이유에 대해서 납득이 되기 시작했다.
왜 이터러블을 사용했었고, go, pipe 등등이 왜 필요했었는지 말이다. 그리고 지연평가, 비동기를 다룰려고 했는지 말이다.

함수형 프로그래밍, 모나드.... 솔직히 어려운 개념이다. 지금도 100% 이해가 안가만 자주 블로그 글을 읽어보면서 곱씹다 보면 익숙해질 날이 오지 않을까 ㅎ
